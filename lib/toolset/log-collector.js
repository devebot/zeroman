...
var debugx = require('debug')('zeroman:log-collector');
var debug0 = require('debug')('stupid:zeroman:log-collector');
...
var Server = function Server(params) {
  ...
  restApp.all(appPath, cors(), function(req, res, next) {
    var reqInfo = lodash.pick(req, [
      'url', 'method', 'protocol', 'hostname', 'path', 'originalUrl', 'headers'
    ]);
    if (debug0.enabled) {
      process.nextTick(function() {
        debug0.enabled && debug0('=@ Received request: %s - Information: %s',
            JSON.stringify(req.body), JSON.stringify(reqInfo));
      });
    }
    Promise.resolve()
      .then(function() {
        if (req.body) {
          return updater(req.body, reqInfo);
        } else {
          return Promise.reject('invalid log message, body is empty');
        }
      })
      .then(function() {
        debugx.enabled && debugx(' - log message has been pushed');
        res.status(200).send('Ok');
      })
      .catch(function(error) {
        debugx.enabled && debugx(' - log message pushing is failed: %s', JSON.stringify(error));
        res.status(404).send(error);
      })
      .finally(function() {
        debugx.enabled && debugx(' - log message pushing end!');
      });
  });

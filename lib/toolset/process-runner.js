...
var pm2 = require('pm2');
var pinbug = require('debug')('zeroman:process-runner');
var stupid = require('debug')('stupid:zeroman:process-runner');
...
var Constructor = function(kwargs) {
          ...
          pm2.list(function(err, procs) {
            ...
            if (err) {
              ...
              if (names instanceof Array) {
                procs = lodash.filter(procs, function(proc) {
                  return (names.indexOf(proc.name) >= 0);
                });
              }
              onResolved(lodash.map(procs, extractProcessInfo));
            }
  ...
  this.deleteAll = function(names) {
    ...
    return self.list(names)
      ...
      .then(function(procNames) {
        if (lodash.isEmpty(procNames)) return [];
        return self.delete(procNames);
      })
  }
